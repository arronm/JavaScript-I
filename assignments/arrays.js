...
// This would be better served as a function given an ID to return the requestedCar
const requestedCar = inventory.filter((car) => car.id === 33)[0];
console.log(`Car 33 is a ${requestedCar.car_year} ${requestedCar.car_make} ${requestedCar.car_model}` );
...
let carModels = inventory.map((car) => car.car_model).sort((a, b) => a > b);
...
let oldCars = carYears.reduce((accumulator, year) => accumulator + ( year < 2000 ? 1 : 0 ), 0);
console.log(oldCars);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = inventory.reduce((accumulator, car) => {
